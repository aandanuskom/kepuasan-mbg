...
st.title("Upload Data Training")
st.caption("Kelola dataset komentar untuk training model SVM")
...
with tab1:
    ...
    st.subheader("Upload Dataset Training")
...
with tab2:
    ...
    st.subheader("Tambah Data Training Manual")
...
with tab3:
    ...
    if len(df) == 0:
        ...
        label_baru = st.radio(
            "Ubah label",
            ["PUAS","NETRAL","TIDAK PUAS"],
            index=["PUAS","NETRAL","TIDAK PUAS"].index(
                data_terpilih["label"]
            )
        )
        ...
        if st.button("Update Label"):
            ...
            cursor.execute("""
            UPDATE dataset_training
            SET label=%s
            WHERE id_training=%s
            """,(label_baru,data_terpilih["id_training"]))
        ...
        if st.button("Hapus Data"):
            ...
            id_hapus = df.loc[hapus_no]["id_training"]

            cursor.execute("""
            DELETE FROM dataset_training
            WHERE id_training=%s
            """,(id_hapus,))
